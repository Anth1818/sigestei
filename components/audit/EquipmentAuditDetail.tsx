...
import { EquipmentAuditHistory } from "@/lib/types";
...
interface EquipmentAuditDetailProps {
  auditHistory: EquipmentAuditHistory;
}

export default function EquipmentAuditDetail({
  auditHistory,
}: EquipmentAuditDetailProps) {
  ...
  return (
    <div className="space-y-6 max-w-full overflow-hidden">
      ...
      <Card>
        ...
        <CardContent>
          {auditHistory.assignments.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              No hay asignaciones registradas
            </p>
          ) : (
            <div className="space-y-4">
              {auditHistory.assignments.map((assignment) => (
                <div
                  key={assignment.id}
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
                    <div className="flex-1 space-y-2 min-w-0">
                      <div className="text-sm space-y-1">
                        <div className="font-medium">
                          Usuario: {assignment?.user?.full_name}
                        </div>
                        {assignment.previous_user && (
                          <div className="text-muted-foreground">
                            Anterior: {assignment?.previous_user?.full_name}
                          </div>
                        )}
                      </div>
                      <div className="flex items-start gap-2 text-sm">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium break-words">{assignment?.location}</div>
                          {assignment?.previous_location && (
                            <div className="text-xs text-muted-foreground break-words">
                              Anterior: {assignment?.previous_location}
                            </div>
                          )}
                        </div>
                      </div>
